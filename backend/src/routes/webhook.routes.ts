...
import { verifyMetaSignatureGlobal } from "../middleware/metaSignature.js";
...
function maybeVerifySignatureGlobal(req: any, res: any, next: any) {
 const isProd = (process.env.NODE_ENV ?? "development") === "production";
 const required = isProd ? true : (process.env.META_SIGNATURE_REQUIRED ?? "false") === "true";
 if (!required) return next();

 return verifyMetaSignatureGlobal(req, res, next);
}
...
// POST inbound Meta
webhookRouter.post("/:channel/:integrationId", attachIntegration, maybeVerifySignatureGlobal, async (req: any, res) => {
 const channel = req.params.channel;
 const integrationId = req.params.integrationId;
 ...
 for (const msg of inbound) {
   ...
   // enviar respuesta (incluye dropdowns si tu flow manda message.kind="choice")
   try {
     if (adapter?.enabled) {
       if (result.message?.kind === "choice" && (adapter as any).sendChoice) {
         await (adapter as any).sendChoice(msg.externalUserId, result.message);
