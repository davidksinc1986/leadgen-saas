...
/**
* GET verify webhook for Meta with integrationId
* /webhooks/:channel/:integrationId
*/
...
/**
* POST inbound with integrationId (Meta real)
* /webhooks/:channel/:integrationId
* Signature verified for Meta channels (whatsapp/messenger/instagram)
*/
webhookRouter.post(
 "/:channel/:integrationId",
 async (req, res, next) => {
   // attach integration to req for signature middleware
   const integrationId = req.params.integrationId;
   if (mongoose.isValidObjectId(integrationId)) {
     (req as any)._integration = await Integration.findById(integrationId);
   }
   next();
 },
 verifyMetaSignature((req) => ((req as any)._integration?.appSecretEnc ?? "")),
 async (req, res) => {
   const channel = req.params.channel;
   const integrationId = req.params.integrationId;
   ...
   const integration = (req as any)._integration;
   ...
   // Adapter por integración (WhatsApp real)
   const adapter =
     channel === "whatsapp" ? whatsappAdapterFromIntegration(integration) : null;
   ...
   for (const msg of inbound) {
     ...
     // Responder al usuario (si hay adapter implementado)
     if (adapter?.enabled) {
...
/**
* Mantener tu endpoint MVP anterior (webchat/header-based)
* /webhooks/:channel  (como ya lo usabas)
*/
